...
- 每片拼圖具有凹凸契合的 tab/blank 邊緣，形狀由演算法隨機生成；拼圖片之間繪有**縫隙線**，模擬實體拼圖的物理質感
- 抓住拼圖片時顯示**白色邊框**；拖放時提供**吸附**預覽（綠光）與**錯位警示**（紅光）
- 相鄰片自動**合組**，整組一起拖曳
- Header 燈泡按鈕可切換格線內的淡淡**預覽提示圖**（22% 透明度），幫助定位拼圖片；設定隨存檔紀錄各自保存
- 完成拼圖時先**縮放置中**（200ms 平滑縮放至 150% 並置中格線），再播放**掃光動畫**（斜向光帶從左上掃至右下，約 1.5 秒），完成音效與掃光同步，動畫結束後顯示完成頁面
...
- 首次使用自動啟動引導教學，逐步介紹上傳圖片、選擇難度、裁切區域、遊戲操作等各頁面功能
- 以 **Spotlight 聚焦**（青綠色邊框 + 深色遮罩）引導視線，確保每步驟只關注一個操作目標
- 教學期間不寫入存檔，完成後才一次性儲存快捷設定與草稿，避免污染歷史紀錄
- 既有使用者（已有拼圖紀錄）自動跳過教學
...
- 遊戲畫面支援**雙指 pinch 縮放**（100%–150%）與**拖曳平移**，方便手機操作
